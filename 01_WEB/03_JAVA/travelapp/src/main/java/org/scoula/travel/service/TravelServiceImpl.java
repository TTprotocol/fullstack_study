...
public class TravelServiceImpl implements TravelService {
    ...
    public void printTravels() {
        List<TravelVO> travels = dao.getTraverls();
        printTravels(travels);
    }

    @Override
    public void printTravelByDistinct() {
        List<String> districts = dao.getDistricts();
        printDistricts(districts);

        int ix = getNumber("선택 : ");
        String district = districts.get(ix - 1);
        List<TravelVO> travels = dao.getTraverls(district);
        printTravels(travels);
    }
    ...
    public void printTravelByPage() {
        ...
        List<TravelVO> travels = dao.getTraverls(page);
        printTravels(travels);
    }
